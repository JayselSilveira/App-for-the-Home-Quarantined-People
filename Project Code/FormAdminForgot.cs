...
{
    public partial class FormAdminForgot : Form
    {
        string randomCode;
        public static string to;

        Form parentForm;
        public FormAdminForgot(Form form)
        {
            InitializeComponent();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(textBox1.Text) && !string.IsNullOrEmpty(textBox2.Text))
            {
                try
                {
                    ...
                    OleDbConnection con = new OleDbConnection("Provider = Microsoft.ACE.OLEDB.12.0; Data Source = 'D:\\HomeQuarantinedApp.xlsx'; Extended Properties = Excel 12.0;");
                    con.Open();
                    String sql = "UPDATE [AdminDetails$] SET [Password] = @Password WHERE EmailId = @EmailId";
                    ...
                    while (r.Read())
                    {
                        if (randomCode == (textBox2.Text).ToString() && textBox1.Text == r[3].ToString())
                        {
                            flag = 1;
                            cmd.ExecuteNonQuery();
                            con.Close();
